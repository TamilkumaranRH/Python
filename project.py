...
# Define a Combobox
    combobox = Combobox(frame, style="TCombobox", values=["Option 1", "Option 2", "Option 3"])
    combobox.place(x=360, y=400)
   
    from tkinter import Button
    b2=Button(frame,text="S U B M I T",font=20,bootstyle="success").place(x=360,y=400)
...
def log(root,e1,e2):
    ...
    if Username=="tamil" and Password=="1234":
        showinfo("messagebox","Successful")

    else:    
      showerror("messagebox","Something Error")
...
def click(root):
    ...
    image_label.pack()
    l1=v.Label(root,text="LOGIN FROM",font=20,bootstyle="dark").place(x=120,y=30)
    l2=v.Label(root,text="Username:",font=20,bootstyle="info").place(x=170,y=100)
    e1=v.Entry(root,bootstyle="info")
    e1.place(x=260,y=100)
    l3=v.Label(root,text="Password:",font=20,bootstyle="info").place(x=170,y=150)
    e2=v.Entry(root,bootstyle="info")
    e2.place(x=260,y=150)
    b1=v.Button(root,text="L O G I N",command=lambda:log(root,e1,e2),bootstyle="success").place(x=290,y=200)
    root.mainloop()
